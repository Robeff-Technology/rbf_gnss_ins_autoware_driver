...
: Node("rbf_gnss_ins_driver", options)
{
  ...
  sub_rtcm_ = this->create_subscription<mavros_msgs::msg::RTCM>(
    config_params_.topics_.rtcm_topic_, rclcpp::SensorDataQoS(),
...
void GnssInsDriver::init_publishers()
{
  ...
  qos_profile.liveliness_lease_duration(
    rclcpp::Duration::max());  // Infinite Liveliness Lease Duration

  pub_gnss_ins_orientation_ =
    this->create_publisher<autoware_sensing_msgs::msg::GnssInsOrientationStamped>(
      "/gnss_ins_orientation", qos_profile);
  pub_nav_sat_fix_ = this->create_publisher<sensor_msgs::msg::NavSatFix>(
    config_params_.topics_.nav_sat_fix_topic_, qos_profile);
...
void GnssInsDriver::binary_callback(const uint8_t * data, GnssStreamParser::MessageId id)
{
  ...
  if (GnssStreamParser::MessageId::ECEF == id) {
    ...
    if (Converter::is_ins_active(ins_pva_.ins_status)) {
      ...
      auto orientation_msg =
        converter_->ins_to_orientation_stamped_msg(ins_pva_, config_params_.frames_.gnss_frame_);
